...
base_dir = "/home/onecloud-user/tsm_sample/tsm_sdk/refactored_sp/sp_client_sdk/examples/dummy_data/group_backup_dummy_data"

# Leader file (JSON metadata)
leader_file = os.path.join(base_dir, "leader.json")

# Find all .tar.gz member files
member_files = glob.glob(os.path.join(base_dir, "*.tar.gz"))
...
for member_file in sorted(member_files):
    ...
    member_backup = BackupRequest(
        ...
        PreCompressed=True,  # .tar.gz files are already compressed
        ObjType=ObjType.FILE,
        MountWait=MountWait.SKIP
    )
...
print(f"  - 1 leader (leader.json)")
print(f"  - {len(member_files)} members (.tar.gz files)")
print("="*60)
